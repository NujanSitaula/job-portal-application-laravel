...
@section('page_title', 'All Industries')
        ...
        <div class="card">
            <div class="card-body">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        @foreach ($employerIndustry as $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->name }}</td>
                            <td>
                                <div class="d-flex justify-content-center">
                                <a href="{{ route('admin.industry.edit', $item->id) }}" class="btn btn-sm btn-outline-primary mr-2" title="Edit" data-toggle="tooltip"><i class="mdi mdi-circle-edit-outline"></i> Edit</a>
                                <a href="{{ route('admin.industry.delete', $item->id) }}" class="btn btn-sm btn-outline-danger" title="Delete" data-toggle="tooltip"><i class="mdi mdi-delete-outline"></i> Delete</a>
                                </div>
                            </td>
                        </tr>
                        @endforeach
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0">Add New Industry</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form method="post" action="{{ route('admin.industry.create') }}">
                    @csrf
                    <div class="form-group">
                      <label for="text">Industry Name</label> 
                      <input id="text" name="name" placeholder="Enter Category Name" type="text" class="form-control">
                    </div> 
                    <div class="form-group">
                      <button name="submit" type="submit" class="btn btn-primary form-control">Add Industry</button>
                    </div>
                  </form>
            </div>
        </div>
